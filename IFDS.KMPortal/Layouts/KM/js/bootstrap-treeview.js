! function (a, b, c, d) {
    ...
    var g = function (b, c) {
        return this.$element = a(b), this.elementId = b.id, this.styleId = this.elementId + "-style", this.init(c), {
            ...
            getSelectNode: a.proxy(this.getSelectNode, this)
        }
    };
    g.prototype.init = function (b) {
     ...
     g.prototype.getSelectNode = function (nodeId) {
    ...
    g.prototype.remove = function () {
        ...
        link: '<a href="#" onclick="onAnchorClick(event, this);" class="nav-inner"></a>',
        badge: '<span class="badge"></span>'
    }, g.prototype.css = ".treeview .list-group-item{cursor:pointer}.treeview span.indent{margin-left:10px;margin-right:10px}.treeview span.icon{}.treeview .node-disabled{color:silver;cursor:not-allowed}", g.prototype.getNode = function (a) {
        return this.nodes[a]
...
function onAnchorClick(event, node) {

    //var treeViewID = node.closest("div").id;
    var treeViewID = $(node).closest("div")[0].id;
    var treeViewObject = '#' + treeViewID;

    var checkedNodes = $(treeViewObject).treeview('getChecked');
    if (checkedNodes.length == 0) {
        var nodeID = node.parentNode.getAttribute("data-nodeid")
        var currentNode = $(treeViewObject).treeview('getSelectNode', nodeID);
        checkedNodes = [currentNode];
    }
    var siteurl = _spPageContextInfo.webServerRelativeUrl;
    var myobj = new Object();
    if (treeViewID == 'ProductTreeview')
    { myobj = { "k": "*", "r": [{ "n": "owstaxIdProductTags", "t": [], "o": "OR", "k": false, "m": {} }] }; }
    else
    { myobj = { "k": "*", "r": [{ "n": "owstaxIdDepartmentTags", "t": [], "o": "OR", "k": false, "m": {} }] }; }

    for (var i = 0; i < checkedNodes.length ; i++) {
        myobj.r[0].t[i] = "string(" + '"#0' + checkedNodes[i].Value + '"' + ')';
        myobj.r[0].m["string(" + '"#0' + checkedNodes[i].Value + '"' + ')'] = checkedNodes[i].text;
    }
    var redirecturl = encodeURIComponent(JSON.stringify(myobj));
    if (siteurl != '/') {
        window.location.href = siteurl + "/Pages/SharePointSearchResults.aspx?#Default=" + redirecturl;
    }
    else {
        window.location.href = "/Pages/SharePointSearchResults.aspx?#Default=" + redirecturl;
    }
}